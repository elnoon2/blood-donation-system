...
-- Tables are now permanent. Drops removed for safety.
--
-- NOTE: a stray `ALTER TABLE notifications ADD (is_read ...)` used to live here
-- between the header comments and the first CREATE TABLE. It ran before the
-- table existed, raised ORA-00942, and was only silenced by
-- spring.sql.init.continue-on-error=true. The column is now declared inline in
-- `CREATE TABLE notifications` (~ line 121) so the ALTER is redundant. Removed.
